    ...
    <div className="space-y-4">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-9 pr-9 py-2.5 text-sm border border-border-light rounded-xl bg-surface focus:outline-none focus:ring-2 focus:ring-cyan/20 focus:border-cyan transition-colors placeholder:text-text-secondary"
        />
        {searchQuery && (
          <button
      ...
      <div className="flex items-center gap-1.5 p-1.5 bg-surface border border-border-light rounded-xl w-full min-w-0">
            ...
            (status) => (
              <Tooltip key={status} content={statusIcons[status].label} position="bottom">
                <button
                  ...
                  className={cn(
                    "p-2 transition-all duration-200 rounded-lg border border-transparent",
                    statusFilter === status
                      ? "bg-gradient-to-b from-white to-surface-filter text-heading-1 shadow-[0_2px_4px_0_rgba(63,140,156,0.15)] border-border-light"
        ...
        {/* Separator */}
        <div className="w-px h-6 bg-border-light mx-1 shrink-0" />
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto min-w-0 scrollbar-none">
          {(Object.keys(typeLabels) as NotificationFilterType[]).map((type) => (
            <button
              ...
              className={cn(
                "px-3 py-1.5 text-xs font-medium transition-all duration-200 rounded-lg whitespace-nowrap shrink-0 border border-transparent",
                typeFilter === type
                  ? "bg-gradient-to-b from-white to-surface-filter text-heading-1 shadow-[0_2px_4px_0_rgba(63,140,156,0.15)] border-border-light"
                  : "text-text-secondary hover:text-cyan hover:bg-cyan-light/50"
              )}
